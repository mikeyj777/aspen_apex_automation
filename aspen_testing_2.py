...
def recurse_that_node(curr_node, prefix = 'Data'):
...
# Use Aspen Plus with WILS-HOC property method for VLE calculations
    ...
    def setup_simulation(self):
        ...
        with App(visible=False) as sim:
            ...
            time.sleep(2)
            data = sim.getNode(r"\Data")
            recurse_that_node(data)
            # components = sim.getNode(r"\Data\Components")
            # specs = sim.getNode(r"\Data\Components\Specifications")
            # comp_specs = sim.getNode(r"\Data\Components\Specifications\Input")

            # Setup the simulation components and property method

            with open('aspen_data_tree.txt', 'w') as file:
    ...
    def _add_components(self, sim):
        ...
        try:
            # Navigate to components specification input
            comp_specs = sim.getNode(r"\Data\Components\Specifications\Input")

            if comp_specs:
                # Get the ID node for component specifications
                comp_id_node = comp_specs.getChild("ID")

                if comp_id_node and comp_id_node.hasChildren():
                    # Add acetic acid as first component
                    acetic_element = comp_id_node.getChild(0)
                    if acetic_element:
                        acetic_element.value = "ACETIC-ACID"

                    # Add water as second component
                    water_element = comp_id_node.getChild(1)
                    if water_element:
                        water_element.value = "WATER"

            # Verify components were added
            components = sim.getComponents()
            comp_names = [str(comp) for comp in components]
            print(f"✓ Components added: {comp_names}")
    ...
    def _set_property_method(self, sim):
        ...
        try:
            # Navigate to property methods specification
            prop_methods = sim.getNode(r"\Data\Properties\Specifications\Input")

            if prop_methods:
                # Get the methods node
                methods_node = prop_methods.getChild("METHODS")

                if methods_node and methods_node.hasChildren():
                    # Set first property method to WILS-HOC
                    first_method = methods_node.getChild(0)
                    if first_method:
                        first_method.value = "WILS-HOC"
                        print("✓ Property method set to WILS-HOC")
                else:
                    print("Warning: Could not access methods node")
            else:
                print("Warning: Could not access property methods node")
    ...
    def _create_flash_block(self, sim):
        ...
        try:
            # Navigate to blocks section to potentially add a flash block
            # For now, we'll rely on Aspen's built-in property calculations
            blocks_node = sim.getNode(r"\Data\Blocks")

            # We'll use Aspen's property calculation methods directly
            # rather than creating explicit blocks
    ...
    def _extract_parameters(self, sim):
        ...
        try:
            # Extract pure component parameters (PLXANT)
            ...
            if plxant_sets and len(plxant_sets) > 0:
                ...
                print("Warning: No PLXANT parameter sets found")
            ...
            if wilson_sets and len(wilson_sets) > 0:
                ...
                print("Warning: No Wilson parameter sets found")
...
if __name__ == '__main__':
    ...
    print('Creates new simulation and uses the Aspen built-in HOC calculations')
    print('=' * 60)
